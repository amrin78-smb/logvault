...
const CACHE_TTL   = 5 * 60 * 1000; // 5 minutes (matches the suite's 5-min dynamic-settings cadence)

async function fetchLicense() {
  const hubUrl = process.env.NOCVAULT_HUB_URL ||
...
function getLicenseState(license) {
  ...
  if (status === 'active' && Array.isArray(license.modules) && license.modules.length > 0 && !license.modules.includes('logvault')) {

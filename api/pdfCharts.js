 ...
 *
 * No new npm dependency, no headless browser. Draws a utilization-over-time
 * chart entirely within a caller-supplied box, safe for portrait reports.
 *
 * Ported verbatim from DDIVault's api/pdfCharts.js (portable, no app-specific
 * coupling). Reserved for a future single-series percentage/time-axis chart
 * (e.g. a per-device or per-site trend block) — LogVault's Phase 1 reports
 * render their multi-series/bar `charts[]` payload via the generic
 * `drawChart` ChartSpec renderer in reports.js instead (see that file's
 * comment for why).
 *
 * Public contract:

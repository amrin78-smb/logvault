    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginBottom: 8, flexShrink: 0 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 7, height: 7, borderRadius: 2 /* intentional: 7x7 legend swatch — --radius-sm (6px) would render it as a circle */, background: 'var(--tint-danger)', border: '1px solid var(--tint-danger-fg)' }} />c = critical
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 7, height: 7, borderRadius: 2 /* intentional: 7x7 legend swatch — --radius-sm (6px) would render it as a circle */, background: 'var(--tint-warn)', border: '1px solid var(--tint-warn-fg)' }} />e = error
        </span>
      </div>
            ...
            <div key={row.vendor} onClick={() => onVendorClick && onVendorClick(row.vendor)}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 7, height: 7, borderRadius: 2 /* intentional: 7x7 vendor swatch — --radius-sm (6px) would render it as a circle */, background: color }} />
                ...
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  {crit > 0 && <span title={`${crit.toLocaleString()} critical`} style={{ fontSize: 'var(--text-xs)', color: 'var(--tint-danger-fg)', background: 'var(--tint-danger)', border: '1px solid var(--tint-danger)', padding: '1px 4px', borderRadius: 'var(--radius-sm)', fontWeight: 600 }}>{crit}c</span>}
                  {err  > 0 && <span title={`${err.toLocaleString()} error${err === 1 ? '' : 's'}`} style={{ fontSize: 'var(--text-xs)', color: 'var(--tint-warn-fg)', background: 'var(--tint-warn)', border: '1px solid var(--tint-warn)', padding: '1px 4px', borderRadius: 'var(--radius-sm)', fontWeight: 600 }}>{err}e</span>}

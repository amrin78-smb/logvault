...
function StatusPill({ value, label, warn = 1, danger = 5, inverse = false }: {
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 8,
      ...
      <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginTop: 4, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</div>
    </div>
  );
}
...
function LinkBadge({ state }: { state: string }) {
  ...
  return <span style={{ color: '#9ca3af', fontSize: 'var(--text-xs)' }}>{state || '—'}</span>;
}
        ...
        <>
          ...
          {activeSection === 'overview' && summary && (
            <>
              ...
              {/* Quick device status */}
              <div style={CARD}>
                <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Device Status at a Glance</div>
                <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>Devices that have sent logs in the last 24 hours</div>
                    ...
                    return (
                      <div key={i} style={{ background: bg, border: `1px solid ${border}`,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: '#1a202c',
                            ...MONO, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {d.host}
                          </span>
                        </div>
                        <div style={{ fontSize: 'var(--text-xs)', color: '#718096', display: 'flex', justifyContent: 'space-between' }}>
          ...
          {activeSection === 'devices' && (
            <div style={CARD}>
              <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Device Status</div>
                  ...
                  {deviceStatus.map((d, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid #f0f2f5',
                        ...
                        <td style={{ ...TD, ...MONO, fontSize: 'var(--text-xs)', color: silent ? '#dc2626' : '#718096' }}>
          ...
          {activeSection === 'interfaces' && (
            <>
              ...
              {flaps.length > 0 && (
                <div style={CARD}>
                  <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Interface Flap Summary</div>
                  <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>
                    ...
                    <tbody>
                      {flaps.map((f, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f0f2f5',
                          ...
                          <td style={{ ...TD, ...MONO, fontSize: 'var(--text-xs)', color: '#9ca3af' }}>{new Date(f.first_seen).toLocaleTimeString()}</td>
                          <td style={{ ...TD, ...MONO, fontSize: 'var(--text-xs)', color: '#9ca3af' }}>{new Date(f.last_seen).toLocaleTimeString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div style={CARD}>
                <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Interface Event Log</div>
                <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>All interface state changes</div>
                {interfaces.length === 0 ? (
                  <EmptyState title="No data" message="No network health events for the selected time range." />
                ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--text-sm)' }}>
          ...
          {activeSection === 'stp' && (
            <div style={CARD}>
              <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>STP &amp; Loop Events</div>
              <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>
          ...
          {activeSection === 'macflaps' && (
            <div style={CARD}>
              <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>MAC Address Flapping</div>
              <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>
                    ...
                    <tbody>
                      {macFlaps.map((r, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f0f2f5', background: '#fff8f8' }}>
                          ...
                          <td style={{ ...TD, ...MONO, fontSize: 'var(--text-xs)', color: '#9ca3af' }}>{new Date(r.first_seen).toLocaleTimeString()}</td>
                          <td style={{ ...TD, ...MONO, fontSize: 'var(--text-xs)', color: '#9ca3af' }}>{new Date(r.last_seen).toLocaleTimeString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeSection === 'routing' && (
            <div style={CARD}>
              <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Routing Protocol Events</div>
              <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>OSPF, BGP, and EIGRP neighbor state changes</div>
              {routing.length === 0 ? (
                <div style={{ padding: '32px 0', textAlign: 'center', color: '#16a34a', fontSize: 'var(--text-base)', fontWeight: 500 }}>
          ...
          {activeSection === 'config' && (
            <div style={CARD}>
              <div style={{ fontSize: 'var(--text-md)', fontWeight: 600, color: '#1a202c', marginBottom: 2 }}>Configuration Changes</div>
              <div style={{ fontSize: 'var(--text-xs)', color: '#718096', marginBottom: 16 }}>

...
(c) CountryFlag  ({ code, size?, className? }) — in ThreatIntel.tsx. Renders a VENDORED SVG from public/flags (flag-icons 4x3, svgo'd; 271 files ~1.9 MB on disk, ~7 KB per page actually fetched). Falls back to a country-code chip on a missing file, globe-less blank on a bad code. **Do not go back to emoji:** countryFlag() returns regional-indicator pairs which Chromium on WINDOWS renders as the two ISO letters (Segoe UI Emoji has no flag ligatures) — that is what "SG"/"US" in the Threat Map was. countryFlag() is KEPT because GeoInline/TopTalkers/TopDestinations test its truthiness to decide whether to render at all, and for text-only contexts. public/flags needs its own Cache-Control rule in next.config.js — the blanket no-store would otherwise re-download every flag on every page load.
